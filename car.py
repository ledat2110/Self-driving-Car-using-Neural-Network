...
class Car(object):
	...
	def __init__(self, x, y):
		self.neuralNet = NeuralNetwork(5, 2, [4,3])
	...
	def update(self):
		...
		outputs[0] = min(1, max(outputs[0], -1))
		outputs[1] = min(1, max(outputs[1], -1))
		
		self.direction += (outputs[0] - 0.5) * 20
		self.speed = min(20, outputs[1] * 10)
	...
	def incrementFitness(self):
		self.cookie += 1
		self.fitness += self.cookie + self.speed/10
