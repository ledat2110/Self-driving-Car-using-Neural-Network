...
class NeuralNetwork(object):
	def __init__(self, numInputs, numOutputs, numNeuronsPerHiddenLayer):
		self.numInputs = numInputs
		self.numOutputs = numOutputs
		self.numHiddenLayers = len(numNeuronsPerHiddenLayer)
		self.numNeuronsPerHiddenLayer = numNeuronsPerHiddenLayer
		self.layers = []
	
	def create(self):
		if self.numHiddenLayers > 0:
			self.layers.append(NeuralLayer(self.numNeuronsPerHiddenLayer[0], self.numInputs))
			
			for i in range(1, self.numHiddenLayers):
				self.layers.append(NeuralLayer(self.numNeuronsPerHiddenLayer[i], self.numNeuronsPerHiddenLayer[i-1]))
			
			self.layers.append(NeuralLayer(self.numOutputs, self.numNeuronsPerHiddenLayer[-1]))
		else:
